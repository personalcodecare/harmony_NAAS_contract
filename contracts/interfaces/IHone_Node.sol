...
interface IHoneNode is ICommonStruct {
   ...
   function setMaintenanceFee(uint8[] memory maintenanceFee_) external;

   function setNodePrice(uint8[] memory nodePrice_) external;

   function setRewardPercent(uint8[] memory rewardPercent_) external;

   function setBreakDay(uint8[] memory breakDay_) external;
