...
contract HoneNode is OwnableUpgradeable, ReentrancyGuardUpgradeable, IHoneNode {
   mapping(address => NodeInfo[]) private userInfos;
   uint8[] private breakDays;
   uint8[] private nodePrice;
   uint8[] private rewardPercent;    // * 1e2
   uint8[] private maintenanceFee;
   IGHONE private gHone;

   function initialize() public initializer {
      __Ownable_init();
      breakDays = [100, 87, 75, 58];
      nodePrice = [10, 20, 50, 100];
      rewardPercent = [100, 115, 135, 175];    // * 1e2
      maintenanceFee = [10, 20, 30, 45];
   }

   function setMaintenanceFee(uint8[] memory maintenanceFee_) external onlyOwner override {
      maintenanceFee = maintenanceFee_;
   }

   function setNodePrice(uint8[] memory nodePrice_) external onlyOwner override {
      nodePrice = nodePrice_;
   }

   function setRewardPercent(uint8[] memory rewardPercent_) external onlyOwner override {
      rewardPercent = rewardPercent_;
   }

   function setBreakDay(uint8[] memory breakDay_) external onlyOwner override {
      breakDays = breakDay_;
   }

   function setGHoneAddress(address ghoneAddress_) external onlyOwner {
      gHone = IGHONE(ghoneAddress_);
   }

   function getMaintenanceFee() external view override returns(uint8[] memory) {
      ...
      for (uint256 i = 0; i < userInfos[userAddress_].length; i ++) {
         if (userInfos[userAddress_][i].burned == false) {
            infos[index ++] = NodeDispInfo({
               ...
               offerStatus: userInfos[userAddress_][i].rentStatus.offerStatus,
